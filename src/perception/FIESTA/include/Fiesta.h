     ...
     if (esdf_map!=nullptr) {
          if (global_vis)
               ...
               current_odom << odom_.pose.pose.position.x , odom_.pose.pose.position.y,odom_.pose.pose.position.z;
                    esdf_map->SetUpdateRange(current_odom - parameters_.radius_, current_odom + parameters_.radius_, false);
...
//    ros::Time t1 = ros::Time::now();
     if (esdf_map_->CheckUpdate()) {
          ...
          esdfUpdateCnt_++;
          ROS_WARN_STREAM("FIESTA update time"  << (ros::Time::now() - t1).toSec()<< "s" ;);
#ifdef SIGNED_NEEDED
